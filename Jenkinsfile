         ...
         stage('Upload to AWS') {
              steps {
                  withAWS(region: 'eu-west-1', credentials: '372cdc38-3d47-4319-b9cc-b8d1d8a3d902') {
                   ...
                   echo ${latest_version}
                   aws cloudformation deploy --stack-name test794 --template-file lambda-packaged.yaml --parameter-overrides ParamS3Bucket=us-east-22222222 ParamS3Key=lambda_function.zip LambdaVersion=${latest_version}
                   '''
                  }
              }
         }
