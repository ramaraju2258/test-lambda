pipeline {
     ...
     stages {
         ...
         stage('Upload to AWS') {
              steps {
                  withAWS(region: 'eu-west-1', credentials: '955b46a5-f950-46ea-8a58-f9241d554ea0') {
                  sh 'aws cloudformation deploy --stack-name testnew33 --template-file lambda-packaged.yaml --parameter-overrides ParamS3Bucket=cloudformation-test2258 ParamS3Key=lambda_function.zip --region eu-west-1 --capabilities CAPABILITY_NAMED_IAM'
                  }
              }
         }
     }
