         ...
         stage('Build') {
             steps {
                 ...
                 sh 'zip -r lambda_function.zip index.js lib'     
                      ...
                      s3Upload(pathStyleAccessEnabled: true, payloadSigningEnabled: true, file:'lambda-packaged.yaml', bucket:'us-east-22222222', region:'eu-west-1')
                 }
             }       
         }      
         stage('Upload to AWS') {
              steps {
                  withAWS(region: 'eu-west-1', credentials: '372cdc38-3d47-4319-b9cc-b8d1d8a3d902') {
                   ...
                   echo ${latest_version}
                   aws cloudformation deploy --stack-name test561 --template-file lambda-packaged.yaml --parameter-overrides ParamS3Bucket=us-east-22222222 ParamS3Key=lambda_function.zip LambdaVersion=${latest_version}
                   '''
                  }
              }
         }
